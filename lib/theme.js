...
const semanticTokens = {
  colors: {
    camp: {
      ...
      // Ember dùng cho CHỮ trên nền sáng. #dd8a2e chỉ đạt 2.2-2.6:1 (AA cần
      // 4.5:1) → chữ cam nhỏ (số trạm 10px, chip ngôn ngữ) bị chìm trên nền kem.
      // #945a18 giữ NGUYÊN hue cam 31.5° của ember, đạt AA trên cả 3 nền sáng:
      // sky 4.91:1 · card 5.40:1 · emberSoft 4.64:1. Token `ember` giữ sáng
      // cho decoration (glow lửa trại, sao, gradient trail) vì đó không phải
      // chữ → không chịu ràng buộc contrast text.
      emberText: { default: '#945a18', _dark: '#f2a541' },
...
const buildGlobalStyles = props => ({
  ...
  // Cuộn mượt do Lenis đảm nhận (lib/smooth-scroll.js) — CSS scroll-behavior
  // không dùng vì sẽ xung đột với lerp của Lenis. prefers-reduced-motion →
  // Lenis tự bỏ, trang cuộn native.
  body: {
  ...
  // Chặn trì hoãn 300ms của double-tap-zoom trên mobile Safari cho mọi phần tử
  // tương tác. `:where()` → specificity 0 nên component vẫn override được.
